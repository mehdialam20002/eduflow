# Brief for 30-student-portal-module.md

Title: Student Portal Module
Minimum words: 4500
Web research needed: no

## What this chapter must cover (every item, fully)

This is the PRD chapter for the **Student Portal** module (code SP). Use EXACTLY the module chapter template in the style guide (all H2 sections, in that order) and the ID formats from the canon (SP-S01, SP-US-01, SP-BR-01, SP-AC-01, SP-API-01).

Sources you must read before writing:
- The endpoint registry section for SP in `E:/mysaasschool/docs/src/_api/03-finance-portals-comms.md` — use those exact endpoint IDs, methods, paths and permission keys. Document EVERY endpoint of this module in the summary table, and give full request + success response + error table for at least the 8 most important ones (all of them if the module has 10 or fewer).
- The Prisma models of this module: look them up in `E:/mysaasschool/docs/src/_schema/README.md`, then open the relevant schema files (04-people, 02-auth, 06-timetable-homework) and copy the models and enums EXACTLY into the "Prisma Schema" section (you may shorten long back-relation lists with a comment line, but never rename or invent fields). The "Database Schema" tables must match those models.
...
Domain focus for this module: Student login (under-18 accounts enabled with parent consent), dashboard, timetable, homework list and submission, exam schedule and results, attendance, library books issued, certificate requests, announcements, profile. Restrictions: no fee payment by default (configurable), no personal data editing beyond allowed fields, age-appropriate design, no advertising or tracking. Mobile-width wireframes.
