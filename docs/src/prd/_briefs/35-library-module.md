# Brief for 35-library-module.md
...
This is the PRD chapter for the **Library** module (code LIB). Use EXACTLY the module chapter template in the style guide (all H2 sections, in that order) and the ID formats from the canon (LIB-S01, LIB-US-01, LIB-BR-01, LIB-AC-01, LIB-API-01).

Sources you must read before writing:
- The endpoint registry section for LIB in `E:/mysaasschool/docs/src/_api/04-operations-intelligence.md` — use those exact endpoint IDs, methods, paths and permission keys. Document EVERY endpoint of this module in the summary table, and give full request + success response + error table for at least the 8 most important ones (all of them if the module has 10 or fewer).
...
Domain focus for this module: Catalog with books and copies (accession number, barcode), categories, issue, return and renew with barcode scan UI, due dates and fines (collected as cash or pushed to a fee invoice), member limits by role, reservations, lost and damaged handling, stock verification, bulk import by ISBN list, student portal view, overdue reminders, reports (overdue, most issued, stock value), Librarian custom role.
