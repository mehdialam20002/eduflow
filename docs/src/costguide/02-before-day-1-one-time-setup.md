...
```mermaid
flowchart TD
    A["Buy domains and 2 digital signatures"] --> B["File SPICe+ Part A and Part B"]
    B --> C["Certificate, CIN, PAN and TAN"]
    C --> D["Udyam registration, free, same day"]
    C --> E["Fund the account, then file INC-20A"]
    D --> F["GST registration, free, 3 to 7 working days"]
    D --> G["Trademark at Rs 4,500 a class"]
    F --> H["Buy the laptop and tools on the GSTIN"]
```
