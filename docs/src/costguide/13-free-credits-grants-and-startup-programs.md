...
```mermaid
flowchart TD
    A["A programme offers credits or cash"] --> B{"Does it want equity<br/>or a fee up front?"}
    B -- "yes" --> C["Skip it unless the cheque<br/>is over Rs 25 lakh"]
    B -- "no" --> D{"Would you spend that money<br/>anyway in the next 12 months?"}
    D -- "no" --> E["Note it and apply later<br/>credits expire unused"]
    D -- "yes" --> F{"Can a bootstrapped company<br/>qualify without an investor?"}
    F -- "no" --> G["Park it until the seed round"]
    F -- "yes" --> H["Apply this week"]
```
