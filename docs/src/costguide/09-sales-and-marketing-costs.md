...
```mermaid
flowchart TD
    A["A channel asks for more money"] --> B{"Cost per win under<br/>Rs 6,000 for 60 days?"}
    B -- "no" --> C["Fix the channel or stop it<br/>the same week"]
    B -- "yes" --> D{"Is loaded CAC still<br/>under Rs 12,000?"}
    D -- "no" --> E["Stop. Spend on onboarding<br/>so customers stay longer"]
    D -- "yes" --> F["Release up to Rs 50,000<br/>from the Rs 4.46 lakh reserve"]
```
