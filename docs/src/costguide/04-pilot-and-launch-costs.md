...
```mermaid
flowchart TD
    A["Private Limited company<br/>plus PAN, TAN and GSTIN"] --> B["Four policy pages live<br/>on eduflow.app"]
    A --> C["Meta business verification<br/>free, needs GST certificate"]
    A --> D["DLT principal entity<br/>Rs 5,900 one time"]
    B --> E["Razorpay activation<br/>free, 2% per payment"]
    C --> F["WhatsApp daily limit rises<br/>from 250 to 2,000 users"]
    D --> G["SMS sender ID and<br/>templates approved"]
    E --> H["First paid invoice<br/>January 2027"]
    F --> H
    G --> H
```
