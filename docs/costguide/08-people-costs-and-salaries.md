...
```mermaid
flowchart TD
    A["A role is hurting"] --> B{"Trigger fired?"}
    B -- "no" --> C["Use a freelancer<br/>for one quarter"]
    B -- "yes" --> D{"Payroll stays under<br/>40% of revenue?"}
    D -- "no" --> E["Hire in a Tier 2 city"]
    D -- "yes" --> F{"6 months of loaded<br/>cost in the bank?"}
    F -- "no" --> C
    F -- "yes" --> G["Make the offer"]
```
