...
```mermaid
flowchart TD
    A["Years 1 and 2: the company loses money"] --> B["Stay in the normal regime<br/>zero profit means zero tax"]
    B --> C["Apply for the section 140 certificate"]
    C --> D{"Certificate granted?"}
    D -- "no" --> E["Opt into section 200 at 25.168%<br/>in the first profitable year"]
    D -- "yes" --> F["Use the 3 holiday years<br/>pay only MAT at 14.56%"]
    F --> G["After the holiday, opt into section 200"]
```
