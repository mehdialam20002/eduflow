# Brief for 37-transport-module.md
...
This is the PRD chapter for the **Transport** module (code TRN). Use EXACTLY the module chapter template in the style guide (all H2 sections, in that order) and the ID formats from the canon (TRN-S01, TRN-US-01, TRN-BR-01, TRN-AC-01, TRN-API-01).

Sources you must read before writing:
- The endpoint registry section for TRN in `E:/mysaasschool/docs/src/_api/04-operations-intelligence.md` — use those exact endpoint IDs, methods, paths and permission keys. Document EVERY endpoint of this module in the summary table, and give full request + success response + error table for at least the 8 most important ones (all of them if the module has 10 or fewer).
- The Prisma models of this module: look them up in `E:/mysaasschool/docs/src/_schema/README.md`, then open the relevant schema files (11-operations) and copy the models and enums EXACTLY into the "Prisma Schema" section (you may shorten long back-relation lists with a comment line, but never rename or invent fields). The "Database Schema" tables must match those models.
...
Domain focus for this module: Vehicles with document expiry alerts (insurance, fitness, PUC), drivers and attendants linked to Staff, routes and stops with pickup/drop times and stop-wise fees, student assignment with seat capacity checks, link to Fees for transport charges and mid-year route change proration, daily trip log and bus attendance, parent view of route, stop and driver contact, maintenance log, GPS live tracking as an optional later integration, safety notes.
