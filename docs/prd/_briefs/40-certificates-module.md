# Brief for 40-certificates-module.md
...
This is the PRD chapter for the **Certificates** module (code CRT). Use EXACTLY the module chapter template in the style guide (all H2 sections, in that order) and the ID formats from the canon (CRT-S01, CRT-US-01, CRT-BR-01, CRT-AC-01, CRT-API-01).

Sources you must read before writing:
- The endpoint registry section for CRT in `E:/mysaasschool/docs/src/_api/04-operations-intelligence.md` — use those exact endpoint IDs, methods, paths and permission keys. Document EVERY endpoint of this module in the summary table, and give full request + success response + error table for at least the 8 most important ones (all of them if the module has 10 or fewer).
...
Domain focus for this module: Templates with merge fields (bonafide, transfer certificate, character, course completion, merit, fee certificate, custom), request, approve and issue flow, serial numbers from NumberSequence, QR code with a public verification page, signatories, bulk issue, revoke and reissue, transfer certificate rules (dues clearance check, student status becomes transferred), PDF generation through the queue.
