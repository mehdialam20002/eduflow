...
This is the PRD chapter for the **Hostel** module (code HST). Use EXACTLY the module chapter template in the style guide (all H2 sections, in that order) and the ID formats from the canon (HST-S01, HST-US-01, HST-BR-01, HST-AC-01, HST-API-01).

Sources you must read before writing:
- The endpoint registry section for HST in `E:/mysaasschool/docs/src/_api/04-operations-intelligence.md` — use those exact endpoint IDs, methods, paths and permission keys. Document EVERY endpoint of this module in the summary table, and give full request + success response + error table for at least the 8 most important ones (all of them if the module has 10 or fewer).
...
Domain focus for this module: Hostels, floors, rooms and beds, allocation, transfer and vacate, hostel fee linkage, night roll-call attendance, visitor log, leave/outpass with parent approval through the Parent Portal, room inventory pointer, warden custom role, occupancy and vacancy reports, gender rules and safety notes.
