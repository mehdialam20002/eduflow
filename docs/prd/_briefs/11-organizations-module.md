# Brief for 11-organizations-module.md
...
This is the PRD chapter for the **Organizations** module (code ORG). Use EXACTLY the module chapter template in the style guide (all H2 sections, in that order) and the ID formats from the canon (ORG-S01, ORG-US-01, ORG-BR-01, ORG-AC-01, ORG-API-01).

Sources you must read before writing:
- The endpoint registry section for ORG in `E:/mysaasschool/docs/src/_api/01-platform-people.md` — use those exact endpoint IDs, methods, paths and permission keys. Document EVERY endpoint of this module in the summary table, and give full request + success response + error table for at least the 8 most important ones (all of them if the module has 10 or fewer).
...
Domain focus for this module: Tenant lifecycle: self-signup, email/phone verification, onboarding wizard (type school or coaching, country, currency, timezone, first academic year, first campus), trial, plans and subscription, plan limits and PLAN_LIMIT_REACHED behaviour, subscription invoices with GST, add-on purchases, suspension for non-payment and reactivation, branding and subdomain, organization profile, SUPER_ADMIN platform console (list tenants, usage, impersonation with audit trail), cancellation with data export and deletion schedule.
